...
- ✅ **CL4** **games carry no genre, year or publisher, so there is no axis to group the grid by** — All 168 entries carry a genre, a publisher and a release year, the year read off the store pages covers.csv already records (design recorded in `scripts/build_dataset.py`).
