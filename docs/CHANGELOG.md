...
- ✅ **CL3 (166 of 184 titles)** **no title says how many players fit on the couch, or whether the screen splits or is shared** — data/coop.csv gives max players, screen mode and scope for 166 of 184 titles, under a closed vocabulary the builder refuses to widen; 34 rows were verified listing by listing.
- ✅ **CL4 (genre on 179 of 184 titles)** **games carry no genre, year or publisher, so there is no axis to group the grid by** — data/catalog.csv gives one genre from a closed ten-label vocabulary for 179 of 184 titles, plus the publisher on 177, so the grid has two axes to group by.
